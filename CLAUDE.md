...
└── models.py           #   Task, ActivityLog, Comment, User dataclasses (string ids); Status/ActivityType/ActivityAction enums; Project (dead)
...
- **`get_task` embeds via one query.** A single SurrealQL `SELECT *, (SELECT … WHERE task = $t) AS activities, (SELECT … WHERE task = $t) AS comments FROM $t` returns the task row with embedded `activities` and `comments`, matching the Dart `Task` model (the old version did three separate selects). `list_tasks` does not embed them.
